 ...
 *
 * Every Polymarket response is treated as untrusted JSON of unknown shape. The
...
/**
 * Coerce to a finite number, or `null`. Accepts numeric strings (Gamma/CLOB
 * frequently return prices and volumes as strings).
 */
...
/**
 * Parse a Gamma "stringified array" field.
 *
 * Gamma returns `outcomes`, `outcomePrices`, and `clobTokenIds` as JSON-encoded
 * strings (e.g. `'["Yes","No"]'`). This accepts either the already-parsed array
 * or the stringified form, and returns `[]` on anything malformed (never
 * throws).
 */
 ...
 * year-2001 millisecond threshold are treated as milliseconds). Returns `null`
...
function epochToIso(epoch: number): string | null {
  ...
  // Values below ~1e12 are seconds (year-2001 cutoff in ms is ~1e12).
