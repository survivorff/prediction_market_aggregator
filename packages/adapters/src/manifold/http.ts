/**
 * Injectable HTTP transport for the Manifold adapter.
 *
 ...
 * so the adapter is fully unit-testable WITHOUT real network calls (the
 * Manifold adapter is REST-only, so HTTP is its single transport).
 *
 * This mirrors the Polymarket adapter's `http.ts` so each adapter folder stays
...
export function createFetchHttpClient(fetchImpl?: FetchLike): HttpClient {
  ...
  if (typeof doFetch !== "function") {
    throw new Error("No fetch implementation available; pass one via ManifoldAdapter options.http");
  }
